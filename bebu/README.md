...
| ---------- | ------------------------------------------------------ | ----------------------------- |
| `app/`     | Flutter user + caller ("host") app                     | Android APK / AAB, iOS IPA    |
| `backend/` | REST + Socket.IO API, ZegoCloud tokens, payments, chat | `bebu-backend` container      |
| `admin/`   | Admin panel (Next.js 15)                               | `bebu-admin` container        |
| `deploy/`  | `install.sh`, Docker Compose, Caddy, `.env` template, DB seed | `bebu-mongo`, `bebu-edge` |
...
```
phone ──HTTPS──▶ bebu-edge (Caddy, auto TLS) ──▶ bebu-backend :5000  (/api, /storage, /socket.io, /gifts, /avatar-studio)
                                             └──▶ bebu-admin   :5001  (everything else)
browser ─┘                                        bebu-backend ──▶ bebu-mongo
```

Authentication is Firebase on every surface:

- App: Firebase Auth (quick/anonymous, Google, phone, email) → ID token sent as
...
- Backend verifies tokens with the Firebase service account stored in the
  `Setting` document. On first boot it is read from
  `deploy/firebase-service-account.json` (written by `install.sh`); later it
  can be replaced in Admin → Settings → General.

Firebase project in use: `neocat-ceae7`, Android package / iOS bundle id
`in.bebuapp.app`.

## Deploying the backend + admin

Fresh server: `cd deploy && ./install.sh` — see `INSTALL.md`. The installer
writes `deploy/.env`, builds the images, seeds starter content and creates the
first admin.

Manual equivalent:

```bash
rsync -az --exclude node_modules --exclude .next bebu/ root@HOST:/opt/bebu/
cd /opt/bebu/deploy
cp .env.example .env                       # fill DOMAIN, PUBLIC_URL, secrets, Firebase web config
cp ~/firebase-service-account.json .       # service-account key from Firebase console
docker compose up -d --build
docker compose exec -T mongo mongosh --quiet bebu < seed/seed.js
docker compose exec backend node scripts/create-admin.js you@example.com 'Password'
```

Two modes, chosen in `.env`:

- **Standalone** (default): `bebu-edge` binds 80/443 and gets certificates
  for `DOMAIN` automatically.
- **Edge** (`COMPOSE_FILE=docker-compose.yml:docker-compose.edge.yml`,
  `EDGE_NETWORK=…`): no public ports; an existing proxy on the host forwards
  `DOMAIN` to `${STACK_NAME}-edge:80`. The VPS runs this way behind its shared
  Caddy with `STACK_NAME=bebuapp`.

Container names are `${STACK_NAME}-*` (default `bebu-*`) because other
projects on a shared network may also have services called `admin`/`backend`.
...
Android `ic_notification` silhouettes, the admin `bebu-logo.png` / favicon,
