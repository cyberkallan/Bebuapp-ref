...
describe('DevTokenVerifier', () => {
  ...
  it('round-trips an encoded dev identity', async () => {
    ...
    expect(identity).toMatchObject({ uid: 'u1', email: 'u1@example.com', provider: 'phone', emailVerified: true });
  });
  ...
  it('rejects anything that is not a dev token', async () => {
    ...
    await expect(verifier.verify('dev:not-base64-json')).rejects.toBeInstanceOf(TokenVerificationError);
    await expect(
