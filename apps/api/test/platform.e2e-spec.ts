...
describe('platform pipeline (e2e)', () => {
  ...
  describe('tenant resolution', () => {
    ...
    it('rejects unknown and malformed tenant keys', async () => {
      expect(
        (await http().get('/api/v1/tenant/config').set('x-tenant-key', 'ghost').expect(404)).body
          .code,
      ).toBe('TENANT_NOT_FOUND');
      expect(
        (await http().get('/api/v1/tenant/config').set('x-tenant-key', 'Not Valid').expect(400))
          .body.code,
      ).toBe('TENANT_NOT_FOUND');
    });
  ...
  describe('authentication & provisioning', () => {
    it('requires a bearer token and rejects invalid ones with stable codes', async () => {
      expect(
        (await http().get('/api/v1/users/me').set('x-tenant-key', 'bebu').expect(401)).body.code,
      ).toBe('AUTH_REQUIRED');
      expect(
        (
          await http()
            .get('/api/v1/users/me')
            .set('x-tenant-key', 'bebu')
  ...
  describe('admin RBAC & tenant isolation', () => {
    ...
    it('scopes tenant admins to their own tenant and forbids privileged actions', async () => {
      ...
      await http()
        .get(`/api/v1/admin/tenants/${other.id}`)
        .set('authorization', adminToken('bebu-admin'))
        .expect(403);
