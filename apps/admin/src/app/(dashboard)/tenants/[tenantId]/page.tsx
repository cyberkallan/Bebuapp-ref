...
export async function generateMetadata({
  params,
}: PageProps<'/tenants/[tenantId]'>): Promise<Metadata> {
...
export default async function TenantDetailPage({ params }: PageProps<'/tenants/[tenantId]'>) {
  ...
  const t = result.data;
  const enabledFlags = Object.entries(t.featureFlags)
    .filter(([, on]) => on)
    .map(([k]) => k);
  const disabledFlags = Object.entries(t.featureFlags)
    .filter(([, on]) => !on)
    .map(([k]) => k);
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Applied by the mobile app at launch from the public tenant config.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Callers may set their own rates within the bounds the tenant allows.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
              ...
              <Fact
                ...
                value={formatBps(t.pricing.platformCommissionBps)}
              />
              <Fact
                label="Audio rate"
                value={`${t.pricing.defaultAudioRatePerMinute} coins / min`}
              />
              <Fact
                label="Video rate"
                value={`${t.pricing.defaultVideoRatePerMinute} coins / min`}
              />
              <Fact
                label="Minimum balance to call"
                value={formatCoins(t.pricing.minimumBalanceToCall)}
              />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Server-side switches; the app reads them from the tenant config.
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Identifiers used for IAP verification and links surfaced in the app.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <dl className="grid gap-3 text-sm">
              <Fact
                ...
                value={t.androidPackageName ?? 'Not set'}
                mono={Boolean(t.androidPackageName)}
              />
              <Fact
                label="iOS bundle id"
                value={t.iosBundleId ?? 'Not set'}
                mono={Boolean(t.iosBundleId)}
              />
              <Fact
                label="Supported countries"
                value={
                  t.supportedCountries.length > 0
                    ? t.supportedCountries.join(', ')
                    : 'All countries'
                }
