          ...
          <Table>
            ...
            <TableBody>
              {tenants.data.map((t) => (
                <TableRow key={t.id}>
                  <TableCell>
                    <Link href={`/tenants/${t.id}`} className="flex items-center gap-3 hover:underline">
                      <span
                        aria-hidden
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate font-mono text-xs text-muted-foreground">{t.key}</span>
                      </span>
                    </Link>
                  </TableCell>
                  <TableCell>
                    <StatusPill tone={t.status === "ACTIVE" ? "success" : "warning"}>{t.status.toLowerCase()}</StatusPill>
                  </TableCell>
                  <TableCell className="hidden md:table-cell">
                    <div className="space-y-0.5 font-mono text-xs">
                      <p className="truncate">{t.androidPackageName ?? <span className="text-muted-foreground">no Android id</span>}</p>
                      <p className="truncate">{t.iosBundleId ?? <span className="text-muted-foreground">no iOS id</span>}</p>
                    </div>
                  </TableCell>
                  <TableCell className="hidden lg:table-cell text-xs">
                    <p>
                      {t.pricing.defaultAudioRatePerMinute} / {t.pricing.defaultVideoRatePerMinute} coins·min
                    </p>
                    <p className="text-muted-foreground">{formatBps(t.pricing.platformCommissionBps)} commission</p>
                  </TableCell>
