      ...
      <div className="w-full max-w-md space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            {adminConfig.authMode === "dev" ? (
              ...
              <p className="rounded-lg border bg-muted px-3 py-2 text-sm text-muted-foreground">
                Firebase sign-in for the admin panel is scheduled for the authentication stage. Until then,
                set <code className="font-mono">ADMIN_AUTH_MODE=dev</code> in non-production environments.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <div className="flex items-center justify-between rounded-lg border bg-card px-3 py-2 text-xs text-muted-foreground">
          ...
          <Badge variant={apiUp ? "secondary" : "destructive"}>
            {apiUp ? `reachable · v${status.info.ok ? status.info.data.version : ""}` : "unreachable"}
          </Badge>
        </div>
      </div>
    </main>
